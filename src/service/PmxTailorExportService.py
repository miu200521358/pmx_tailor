...
class PmxTailorExportService:
    ...
    def create_rigidbody(
        ...
        for base_map_idx, regist_bones in all_regist_bones.items():
            ...
            v_yidxs = list(range(regist_bones[:-1, :].shape[0]))
            ...
            for v_yidx, v_xidx in zip(np.where(regist_bones[:-1, :])[0], np.where(regist_bones[:-1, :])[1]):
                ...
                prev_above_vv = (
                    virtual_vertices.get(tuple(vertex_maps[prev_map_idx][v_yidx, prev_xidx]), None)
                    if vertex_maps[prev_map_idx].shape[0] > v_yidx and vertex_maps[prev_map_idx].shape[1] > prev_xidx
                    else None
                )
                prev_now_vv = (
                    virtual_vertices.get(tuple(vertex_maps[prev_map_idx][v_yidx, prev_xidx]), None)
                ...
                prev_below_vv = (
                    virtual_vertices.get(tuple(vertex_maps[prev_map_idx][below_yidx, prev_xidx]), None)
                    if vertex_maps[prev_map_idx].shape[0] > below_yidx
                    and vertex_maps[prev_map_idx].shape[1] > prev_xidx
                    else None
                )
                now_above_vv = virtual_vertices.get(tuple(vertex_map[v_yidx, v_xidx]), None)
                now_below_vv = virtual_vertices.get(tuple(vertex_map[below_yidx, v_xidx]), None)
                next_above_vv = (
                    virtual_vertices.get(tuple(vertex_maps[next_map_idx][v_yidx, next_xidx]), None)
                    if vertex_maps[next_map_idx].shape[0] > v_yidx and vertex_maps[next_map_idx].shape[1] > next_xidx
                    else None
                )
                next_now_vv = (
                    virtual_vertices.get(tuple(vertex_maps[next_map_idx][v_yidx, next_xidx]), None)
                ...
                next_below_vv = (
                    virtual_vertices.get(tuple(vertex_maps[next_map_idx][below_yidx, next_xidx]), None)
                    if vertex_maps[next_map_idx].shape[0] > below_yidx
                    and vertex_maps[next_map_idx].shape[1] > next_xidx
                    else None
                )
                ...
                if rigidbody_shape_type == 0:
                    ...
                    shape_size = MVector3D(
                        x_size * (0.27 if prev_connected or next_connected else 0.5),
                        max(0.25, (y_size * 0.5)),
                ...
                if (
                    next_above_bone
                    and next_above_bone != now_above_bone
                    and prev_above_bone
                    and prev_above_bone != now_above_bone
                ):
                    y_direction_pos = (next_above_bone.position - prev_above_bone.position).normalized()
                elif next_above_bone and next_above_bone != now_above_bone and now_above_bone:
                    y_direction_pos = (next_above_bone.position - now_above_bone.position).normalized()
                elif prev_above_bone and prev_above_bone != now_above_bone and now_above_bone:
                    y_direction_pos = (now_above_bone.position - prev_above_bone.position).normalized()
                else:
    ...
    def create_vertex_map(
        ...
        for index_idx in model.material_indices[material_name]:
            ...
            for v0_idx, v1_idx, v2_idx in zip(
                ...
                if np.isclose(material_direction, 0):
                    ...
                    logger.debug(
                        f"HORIZONAL index[{index_idx}], v0[{v0.index}:{v0_key}], mn[{mean_pos.to_log()}], sn[{surface_normal.to_log()}], pa[{parent_bone.position.to_log()}], pd[{parent_intersect.to_log()}], dot[{round(normal_dot, 5)}]"
                    )
                else:
                    logger.debug(
                        f"TILT index[{index_idx}], v0[{v0.index}:{v0_key}], mn[{mean_pos.to_log()}], sn[{surface_normal.to_log()}], pa[{parent_bone.position.to_log()}], pd[{parent_intersect.to_log()}], dot[{round(normal_dot, 5)}]"
                    )
        ...
        while len(edge_vkeys) < len(edge_line_pairs.keys()):
            _, all_edge_lines, edge_vkeys = self.get_edge_lines(
                edge_line_pairs, None, all_edge_lines, edge_vkeys, param_option
            )
        ...
        for n, edge_lines in enumerate(all_edge_lines):
            ...
            for now_edge_key in edge_lines:
                ...
                if material_direction == 0:
                    # 水平の場合
                    # キーの主に動く方向の位置。Z固定
                    target_idx_poses[-1].append(now_edge_pos.data()[2])
                    mean_poses.append(now_edge_pos.data()[2])
                else:
        ...
        # 半分で分ける(下右は大きい方、上左は小さい方)
        for n, edge_lines in enumerate(all_edge_lines):
            if param_option["direction"] in ["下", "右"]:
                if len(np.where(np.array(target_idx_poses[n]) > target_mean)) > 0:
                    for key in np.array(edge_lines)[np.where(np.array(target_idx_poses[n]) > target_mean)]:
                        top_edge_keys.append(tuple(key))
                if len(np.where(np.array(target_idx_poses[n]) < target_mean)) > 0:
                    for key in np.array(edge_lines)[np.where(np.array(target_idx_poses[n]) < target_mean)]:
                        bottom_edge_keys.append(tuple(key))
            else:
                if len(np.where(np.array(target_idx_poses[n]) < target_mean)) > 0:
                    for key in np.array(edge_lines)[np.where(np.array(target_idx_poses[n]) < target_mean)]:
                        top_edge_keys.append(tuple(key))
        ...
        top_edge_median_pos = MVector3D(np.median(top_edge_poses, axis=0))

        bottom_edge_poses = []
        for key in bottom_edge_keys:
            bottom_edge_poses.append(virtual_vertices[key].position().data())

        bottom_edge_median_pos = MVector3D(np.median(bottom_edge_poses, axis=0))
        # # 中央から真後ろ
        # bottom_edge_start_pos = MVector3D(
        #     0, 0, MVector3D(bottom_edge_median_pos * np.abs(base_vertical_axis.data())).length()
        # ) + MVector3D(bottom_edge_median_pos * np.abs(base_vertical_axis.data()))

        # できるだけ評価軸は離して登録する
        if param_option["direction"] in ["下"]:
            top_edge_median_pos.setY(np.max(top_edge_poses, axis=0)[1])
            bottom_edge_median_pos.setY(np.min(bottom_edge_poses, axis=0)[1])
        elif param_option["direction"] in ["上"]:
            top_edge_median_pos.setY(np.min(top_edge_poses, axis=0)[1])
            bottom_edge_median_pos.setY(np.max(bottom_edge_poses, axis=0)[1])
        elif param_option["direction"] in ["左"]:
            top_edge_median_pos.setX(np.min(top_edge_poses, axis=0)[0])
            bottom_edge_median_pos.setX(np.max(bottom_edge_poses, axis=0)[0])
        elif param_option["direction"] in ["右"]:
            top_edge_median_pos.setX(np.max(top_edge_poses, axis=0)[0])
            bottom_edge_median_pos.setX(np.min(bottom_edge_poses, axis=0)[0])

        # 半径
        top_radius = np.max(np.linalg.norm(np.array(top_edge_poses) - top_edge_median_pos.data(), ord=2, axis=1))
        bottom_radius = np.max(
            np.linalg.norm(np.array(bottom_edge_poses) - bottom_edge_median_pos.data(), ord=2, axis=1)
        )

        logger.info("%s: エッジ上部 中央位置[%s] 半径[%s]", material_name, top_edge_median_pos.to_log(), round(top_radius, 4))
        logger.info(
            "%s: エッジ下部 中央位置[%s] 半径[%s]", material_name, bottom_edge_median_pos.to_log(), round(bottom_radius, 4)
        )

        # # 下部の回転量を計算しておく
        # bottom_degrees = []
        # for bottom_key in bottom_edge_keys:
        #     bottom_degrees.append(
        #         self.calc_arc_degree(
        #             bottom_edge_start_pos,
        #             bottom_edge_median_pos,
        #             virtual_vertices[tuple(bottom_key)].position(),
        #             base_vertical_axis,
        #             base_reverse_axis,
        #         )
        #     )
        ...
        for ti, bottom_key in enumerate(bottom_edge_keys):
            ...
            # 中心から見た処理対象仮想頂点の位置
            bottom_local_pos = (bottom_target_pos - bottom_edge_median_pos) * base_reverse_axis
            # 上部の理想位置をざっくり比率から求めておく
            top_target_pos = top_edge_median_pos + (bottom_local_pos * (top_radius / bottom_radius))
            ...
            logger.info(
                "頂点ルート走査[%s]: 始端: %s -> 終端: %s, スコア: %s",
                f"{(ti + 1):03d}",
                virtual_vertices[vkeys[0]].vidxs() if vkeys else "NG",
                virtual_vertices[vkeys[-1]].vidxs(),
                round(
                    np.average(vscores, weights=list(reversed((np.arange(1, len(vscores) + 1) ** 2).tolist()))),
                    4,
                )
        ...
        tmp_vertex_map = np.full((len(yu), len(xu), 3), (np.nan, np.nan, np.nan))
        registed_vertices = []

        xx = 0
        for x, (vkeys, scores) in enumerate(zip(all_vkeys, all_scores)):
            is_regist = [True for y in vkeys]
            ...
            if not np.count_nonzero(is_regist):
                # 登録対象外の場合、スルー
                continue

            # # デフォルトでは真ん中に配置
            # y_offset = int((tmp_vertex_map.shape[0] / 2) - (len(vkeys) / 2))
            # if x > 0:
            #     # 1以上は隣と繋がってる箇所を探す
            #     target_vkey_y = int(len(vkeys) / 2)
            #     target_vkey = vkeys[target_vkey_y]
            #     connected_vyidxs = []
            #     connected_vposes = []
            #     for vy, vk in enumerate(tmp_vertex_map[:, xx - 1]):
            #         if np.isnan(vk).any():
            #             continue

            #         if target_vkey in virtual_vertices[tuple(vk)].connected_vvs:
            #             connected_vyidxs.append(vy)
            #             connected_vposes.append(virtual_vertices[tuple(vk)].position().data())

            #     if len(connected_vyidxs) > 0:
            #         # 繋がってる箇所がある場合、評価対象と近い方に揃える
            #         y_offset = (
            #             connected_vyidxs[
            #                 np.argmin(
            #                     np.linalg.norm(
            #                         np.array(connected_vposes)
            #                         - virtual_vertices[tuple(target_vkey)].position().data(),
            #                         ord=2,
            #                         axis=1,
            #                     )
            #                 )
        ...
        # 各頂点の結合
        tmp_bone_connected = np.zeros((tmp_vertex_map.shape[0], tmp_vertex_map.shape[1]), dtype=np.int)
        for v_yidx in range(tmp_vertex_map.shape[0]):
            v_xidx = -1
            for v_xidx in range(0, tmp_vertex_map.shape[1] - 1):
                if (
                    not np.isnan(tmp_vertex_map[v_yidx, v_xidx]).any()
                    and not np.isnan(tmp_vertex_map[v_yidx, v_xidx + 1]).any()
                    and tuple(tmp_vertex_map[v_yidx, v_xidx])
                    in virtual_vertices[tuple(tmp_vertex_map[v_yidx, v_xidx + 1])].connected_vvs
                ):
                    # 前の仮想頂点と繋がっている場合、True
                    tmp_bone_connected[v_yidx, v_xidx] = True
                else:
                    tmp_bone_connected[v_yidx, v_xidx] = False

            v_xidx += 1
            if (
                not np.isnan(tmp_vertex_map[v_yidx, v_xidx]).any()
                and not np.isnan(tmp_vertex_map[v_yidx, 0]).any()
                and tuple(tmp_vertex_map[v_yidx, 0])
                in virtual_vertices[tuple(tmp_vertex_map[v_yidx, v_xidx])].connected_vvs
            ):
                # 横の仮想頂点と繋がっている場合
                tmp_bone_connected[v_yidx, v_xidx] = True

        connected_vertex_map = np.copy(tmp_vertex_map)
        bone_connected = np.copy(tmp_bone_connected)
        for v_xidx in range(0, tmp_vertex_map.shape[1]):
            if (
                np.count_nonzero(tmp_bone_connected[:, v_xidx]) < (connected_vertex_map.shape[0] / 2 - 1) * 0.7
                and np.count_nonzero(tmp_bone_connected[:, -1]) > 0
            ):
                # どこか完全に分断されていて、かつ最後が最初と繋がってる箇所がある場合、ぐるっとずらす
                connected_vertex_map = np.full(
                    (tmp_vertex_map.shape[0], tmp_vertex_map.shape[1], 3), (np.nan, np.nan, np.nan)
                )

                connected_vertex_map[:, : (tmp_vertex_map.shape[1] - v_xidx - 1)] = tmp_vertex_map[:, (v_xidx + 1) :]
                connected_vertex_map[:, (tmp_vertex_map.shape[1] - v_xidx - 1) :] = tmp_vertex_map[:, : (v_xidx + 1)]

                bone_connected[:, : (tmp_bone_connected.shape[1] - v_xidx - 1)] = tmp_bone_connected[:, (v_xidx + 1) :]
                bone_connected[:, (tmp_bone_connected.shape[1] - v_xidx - 1) :] = tmp_bone_connected[:, : (v_xidx + 1)]
                break
        ...
        prev_x = 0
        x_count = 0
        for v_xidx in range(connected_vertex_map.shape[1]):
            x_count += 1
            if (
                np.count_nonzero(bone_connected[:, v_xidx]) < (connected_vertex_map.shape[0] / 2 - 1) * 0.7
                or v_xidx == connected_vertex_map.shape[1] - 1
            ):
                logger.info("-- 絶対頂点マップ[%s]: マップ生成", v_xidx + 1)

                # 規定以上横が繋がってない箇所がある・最後の場合、分ける
                vertex_map = np.full((connected_vertex_map.shape[0], x_count, 3), (np.nan, np.nan, np.nan))
                vertex_map = connected_vertex_map[:, prev_x : (prev_x + x_count)]
                ...
                for v_yidx in range(vertex_map.shape[0]):
                    if not np.isnan(vertex_map[v_yidx, :]).any():
                        break
                ...
                remove_xidxs = []
                for v_xidx in range(vertex_map.shape[1]):
                    if np.isnan(vertex_map[:, v_xidx]).all():
                        # 全部nanの場合、削除対象
                        remove_xidxs.append(v_xidx)

                vertex_map = np.delete(vertex_map, remove_xidxs, axis=1)

                # マップ表示
                vertex_display_map = np.full((vertex_map.shape[0], x_count), "None  ")
                for y in range(vertex_map.shape[0]):
                    for x in range(vertex_map.shape[1]):
                        if np.isnan(vertex_map[y, x]).any():
                            continue
                        vv = virtual_vertices[tuple(vertex_map[y, x])]
                ...
                #                     np.argmin(
                #                         np.abs(np.where(~np.isnan(vertex_map[nearest_v_yidx, :, 0]))[0] - v_xidx)
                #                     )
                #                 ]
                ...
                #                 tuple(vertex_map[nearest_above_v_yidx, nearest_v_xidx])
                #             ]
                ...
                logger.info("-- 絶対頂点マップ: %s個目:終了 ---------", v_xidx + 1)
    ...
    def get_edge_lines(
        self, edge_line_pairs: dict, start_vkey: tuple, edge_lines: list, edge_vkeys: list, param_option: dict, loop=0
    ):
        ...
        if not start_vkey:
            if param_option["direction"] == logger.transtext("上"):
                ...
                # X(降順) - Y(降順) - Z(降順)
                sorted_edge_line_pairs = sorted(
                    list(set(edge_line_pairs.keys()) - set(edge_vkeys)), key=lambda x: (-x[0], -x[1], -x[2])
                )
            elif param_option["direction"] == logger.transtext("左"):
                # X(昇順) - Y(降順) - Z(降順)
                sorted_edge_line_pairs = sorted(
        ...
        if param_option["direction"] == logger.transtext("上"):
            ...
            # Y(降順) - Z(降順) - X(降順)
            sorted_edge_line_pairs = sorted(edge_line_pairs[start_vkey], key=lambda x: (-x[1], -x[2], -x[0]))
        elif param_option["direction"] == logger.transtext("左"):
            # Y(降順) - Z(降順) - X(昇順)
            sorted_edge_line_pairs = sorted(edge_line_pairs[start_vkey], key=lambda x: (-x[1], -x[2], x[0]))
        else:
        ...
        for next_vkey in sorted_edge_line_pairs:
            if next_vkey not in edge_vkeys:
                ...
                start_vkey, edge_lines, edge_vkeys = self.get_edge_lines(
                    edge_line_pairs, next_vkey, edge_lines, edge_vkeys, param_option, loop + 1
                )
...
#     dot = MVector3D.dotProduct(Q1.normalized(), u.normalized())
#     if np.isclose(dot, 0):
#         return None
